defmodule CircuitBreaker do
  ...
  def handle_cast({:check_servicies}, state) do
    service_list = :ets.tab2list(:report_service) ++ :ets.tab2list(:graphic_service)
    for service <- service_list do
      ...
      try do
        ...
        HTTPoison.Error ->
          ...
          status = test_server_availability("http://#{address}:#{port}/health", 3)
          ...
          if status == "fail" do
            ...
            :ets.delete(String.to_atom(service_type), address)
